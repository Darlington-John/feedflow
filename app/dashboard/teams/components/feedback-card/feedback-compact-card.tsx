...
import { deleteUserFeedback } from "~/lib/redux/slices/user-feedbacks";

interface props {
  feed: feedback_type;
  setFeedbackCount?: React.Dispatch<React.SetStateAction<number>>;
}
const FeedbackCompactCard = ({ feed, setFeedbackCount }: props) => {
  ...
  const handleDeleteFeedback = async () => {
    ...
    await apiRequest({
      url: `/api/teams/${feed.team._id}/delete-feedback`,
      method: "DELETE",
      ...
      onSuccess: (res) => {
        ...
        setTimeout(() => {
          toggleDeleteOpen();
          setSucessful(false);
          dispatch(deleteUserFeedback({ feedbackId: feed._id }));
          if (setFeedbackCount) {
    ...
    >
      ...
      <div className="flex flex-col  pt-3 gap-2">
        <div className="flex flex-col ">
          <h1 className="text-xs text-silver-blue leading-none">
            You gave a feedback:
          </h1>
      ...
      <ReactionBar feed={feed} toggleMarkOpen={toggleMarkOpen} compact={true} />
      <MarkFeedback
        isActive={markOpen}
        isVisible={markVisible}
        ref={markRef}
...
export default FeedbackCompactCard;
